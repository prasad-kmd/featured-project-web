...
import { siteConfig, ogConfig } from "@/lib/config";

export const runtime = "edge";

export async function GET(req: NextRequest) {
  const { searchParams } = req.nextUrl;
  const title = searchParams.get("title") || `${siteConfig.author}'s Workspace`;
  const description =
      ...
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "32px" }}>
          ...
          >
            ...
            >
              {typeLabels[type] || `${siteConfig.author}'s Workspace`}
            </span>
          </div>
